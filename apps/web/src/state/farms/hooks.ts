...
import { useCakeBusdPrice, useCakeBusdPrice2 } from 'hooks/useBUSDPrice'
...
const coreFarmPIDs = {
  [ChainId.BSC]: [2, 3],
  [ChainId.BSC_TESTNET]: [4, 10],
  [ChainId.GOERLI]: [13, 11],
  [ChainId.ETHEREUM]: [124, 125],
}
...
export const usePriceCakeBusd = ({ forceMainnet } = { forceMainnet: false }): BigNumber => {
  ...
  const [price1, price2] = useCakeBusdPrice2({ forceMainnet })
  return useMemo(() => {
    if (price)
      return new BigNumber(price.toSignificant(6))
    if(price1 && price2)
      return new BigNumber(parseFloat(price1.toSignificant(10)) * parseFloat(price2.toSignificant(10)))
    return BIG_ZERO
  }, [price, price1, price2])
}
