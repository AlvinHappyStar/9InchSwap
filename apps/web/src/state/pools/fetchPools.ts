...
import { ChainDefaultId } from '@pancakeswap/sdk'
...
const startEndBlockCalls = livePoolsWithEnd.flatMap((poolConfig) => {
  return [
    {
      address: getAddress(poolConfig.contractAddress, ChainDefaultId),
      name: 'startBlock',
    },
    {
      address: getAddress(poolConfig.contractAddress, ChainDefaultId),
...
export const fetchPoolsBlockLimits = async () => {
  const startEndBlockRaw = await multicall(sousChefABI, startEndBlockCalls, ChainDefaultId)
...
const poolsBalanceOf = poolsConfig.map((poolConfig) => {
  return {
    ...
    name: 'balanceOf',
    params: [getAddress(poolConfig.contractAddress, ChainDefaultId)],
  }
})

export const fetchPoolsTotalStaking = async () => {
  const poolsTotalStaked = await multicall(erc20ABI, poolsBalanceOf, ChainDefaultId)
...
export const fetchPoolsStakingLimits = async (
  ...
  const poolStakingCalls = validPools
    .map((validPool) => {
      const contractAddress = getAddress(validPool.contractAddress, ChainDefaultId)
