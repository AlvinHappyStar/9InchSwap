...
export const fetchPublicVaultData = async (cakeVaultAddress = cakeVaultV2) => {
  try {
    ...
    const cakeBalanceOfCall = {
      abi: cakeAbi,
      address: CAKE[ChainDefaultId].address,
      name: 'balanceOf',
      params: [cakeVaultV2],
    }
...
export const fetchVaultFees = async (cakeVaultAddress = cakeVaultV2) => {
  try {
    ...
    const [[performanceFee], [withdrawalFee], [withdrawalFeePeriod]] = await multicallv2({ abi: cakeVaultAbi, calls, chainId: ChainDefaultId })
