...
export const fetchPoolsAllowance = async (account) => {
  const calls = nonBnbPools.map((pool) => ({
    ...
    name: 'allowance',
    params: [account, getAddress(pool.contractAddress, ChainDefaultId)],
  }))

  const allowances = await multicall(erc20ABI, calls, ChainDefaultId)
...
export const fetchUserBalances = async (account) => {
  ...
  const tokenBnbBalancesRaw = await multicallv3({ calls: [...tokenBalanceCalls, bnbBalanceCall], chainId: ChainDefaultId })
...
export const fetchUserStakeBalances = async (account) => {
  ...
  const userInfo = await multicall(sousChefABI, calls, ChainDefaultId)
  return fromPairs(
...
export const fetchUserPendingRewards = async (account) => {
  ...
  const res = await multicall(sousChefABI, calls, ChainDefaultId)
