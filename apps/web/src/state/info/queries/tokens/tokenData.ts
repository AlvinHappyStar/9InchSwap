...
import { getMultiChainQueryEndPointWithStableSwap, MultiChainName, multiChainQueryMainToken, multiChainStartBlock } from '../../constant'
...
const TOKEN_AT_BLOCK = (chainName: MultiChainName, block: number | undefined, tokens: string[]) => {
  ...
  const blockString = block ? `block: {number: ${Math.max(block, multiChainStartBlock[chainName])}}` : ``
  return `tokens(
