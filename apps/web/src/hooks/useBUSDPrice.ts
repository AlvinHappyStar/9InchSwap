...
export const useCakeBusdPrice = (
  ...
  const price = usePriceByPairs(BUSD[chainId], cake)
  return price
}

export const useCakeBusdPrice2 = (
  { forceMainnet } = { forceMainnet: false },
): Price<ERC20Token, ERC20Token>[] | undefined => {
  const { chainId } = useActiveChainId()
  // const isTestnet = !forceMainnet && isChainTestnet(chainId)
  // Return bsc testnet cake if chain is testnet
  const cake: Token = CAKE[chainId]
  const price1 = usePriceByPairs(BUSD[chainId], WNATIVE[chainId])
...
export const useBNBBusdPrice = (
  ...
  // const wbnb: Token = isTestnet ? WNATIVE[ChainId.BSC_TESTNET] : WNATIVE[ChainId.BSC]
