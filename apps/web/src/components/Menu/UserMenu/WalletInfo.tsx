...
import { ChainDefaultId, ChainId, WNATIVE } from '@pancakeswap/sdk'
...
const WalletInfo: React.FC<WalletInfoProps> = ({ hasLowNativeBalance, onDismiss }) => {
  ...
  const isBSC = chainId === ChainDefaultId
  const bnbBalance = useBalance({ address: account, chainId: ChainDefaultId })
  const nativeBalance = useBalance({ address: account, enabled: !isBSC })
  const native = useNativeCurrency()
  const wNativeToken = !isBSC ? WNATIVE[chainId] : null
  const wBNBToken = WNATIVE[ChainDefaultId]
      ...
      {/* <Box mb="24px">
        <Flex justifyContent="space-between" alignItems="center" mb="8px">
          <Flex bg={COLORS.BNB} borderRadius="16px" pl="4px" pr="8px" py="2px">
            <ChainLogo chainId={ChainDefaultId} />
            <Text color="white" ml="4px">
              BNB Smart Chain
            </Text>
          </Flex>
          <LinkExternal isBscScan href={getBlockExploreLink(account, 'address', ChainDefaultId)}>
            {getBlockExploreName(ChainDefaultId)}
          </LinkExternal>
        </Flex>
        <Flex alignItems="center" justifyContent="space-between">
